...
//     );
//     console.log("private messages..", messageData);
//     if (recipient) {
...
app.get("/messages", (req, res) => {
  const { sender, recipient, page, pageSize } = req.query;
  const offset = (page - 1) * pageSize;
  const limit = parseInt(pageSize);
  const query =
    "SELECT sender, recipient, message, created_at FROM messages WHERE (sender = ? AND recipient = ?) OR (sender = ? AND recipient = ?) ORDER BY created_at DESC LIMIT ?, ?";
  connection.query(
    query,
    [sender, recipient, recipient, sender, offset, limit],
    (error, results) => {
      if (error) {
        console.error("Error fetching messages:", error);
        res.status(500).send("Error fetching messages");
